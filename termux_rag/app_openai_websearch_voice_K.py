#!/usr/bin/env python3
"""
Voice RAG + OpenAI + 웹검색 통합 시스템 (All-in-One)
하나의 파일로 RAG + OpenAI + 웹검색 + 음성 채팅 모두 실행

주요 기능:
    1. RAG: 저장된 문서에서 관련 정보 검색
    2. 웹 검색: DuckDuckGo를 통한 무료 웹 검색 (API 키 불필요!)
    3. OpenAI: GPT-4o 자연어 답변 생성
    4. Voice: 음성 인식(STT) + 음성 출력(TTS)

사용법:
    pip install flask hnswlib duckduckgo-search openai
    python app_openai_websearch_voice.py
...
try:
    ...
    print("⚠️ openai 라이브러리가 없습니다. 'pip install openai' 로 설치하세요.")
...
# ===== OpenAI API 설정 =====
# 기존 Claude 설정을 OpenAI로 변경!
OPENAI_API_KEY = os.environ.get("OPENAI_API_KEY", "")
OPENAI_MODEL = os.environ.get("OPENAI_MODEL", "gpt-3.5-turbo")  # 또는 "gpt-3.5-turbo"
...
# ===== OpenAI API (웹 검색 결과 포함) =====
def ask_openai_with_web_search(question, context_docs, use_web_search=False):
    """
    OpenAI API 호출 (웹 검색 결과 포함)
    
    핵심 변경점 (Claude → OpenAI):
    - Anthropic API → OpenAI API
    - claude-sonnet-4 → gpt-4o
    - 웹 검색은 DuckDuckGo 사용 (무료!)
    """
    ...
    if use_web_search:
        ...
        if web_sources:
            ...
            for i, source in enumerate(web_sources, 1):
                web_context += f"[웹 {i}] {source['title']}\n{source['snippet']}\nURL: {source['url']}\n\n"
...
웹 검색 결과를 사용한 경우, 출처를 간략히 언급해주세요."""
...
위 문서들과 웹 검색 결과를 참고하여 질문에 답변해주세요."""
    else:
        system_prompt = """당신은 친절한 AI 어시스턴트입니다. 
한국어로 답변해주세요.
웹 검색 결과가 제공된 경우, 이를 참고하여 답변하고 출처를 간략히 언급해주세요."""
...
질문: {question}"""
    ...
    try:
        ...
        if "invalid_api_key" in error_msg.lower() or "401" in error_msg:
            ...
            return f"⚠️ 오류 발생: {error_msg}", []


# ===== HTML 템플릿 (Voice + 웹 검색 UI) =====
MOBILE_APP_HTML = '''
<!DOCTYPE html>
<html lang="ko">
<head>
    ...
    <title>🎤 Voice RAG + OpenAI + 웹검색</title>
    <style>
        * { margin: 0; padding: 0; box-sizing: border-box; -webkit-tap-highlight-color: transparent; }
        ...
        /* 헤더 - OpenAI 그린 그라데이션으로 변경 */
        .header { background: linear-gradient(135deg, #10a37f, #1a7f5a); padding: 15px; text-align: center; }
        ...
        .message.user { background: linear-gradient(135deg, #10a37f, #1a7f5a); align-self: flex-end; border-bottom-right-radius: 5px; }
        ...
        .message-actions button { background: rgba(16,163,127,0.2); border: 1px solid #10a37f; color: #10a37f; padding: 4px 10px; border-radius: 12px; font-size: 0.7rem; cursor: pointer; }
        .message-actions button:hover { background: rgba(16,163,127,0.4); }
        ...
        /* 음성 버튼 */
        .voice-btn { width: 50px; height: 50px; border-radius: 50%; border: none; background: linear-gradient(135deg, #10a37f, #1a7f5a); color: white; font-size: 1.3rem; cursor: pointer; flex-shrink: 0; transition: transform 0.1s; }
        ...
        /* 전송 버튼 */
        .send-btn { padding: 12px 20px; background: linear-gradient(135deg, #10a37f, #1a7f5a); border: none; border-radius: 25px; color: white; font-weight: bold; font-size: 0.9rem; cursor: pointer; }
        ...
        /* 문서 아이템 */
        .doc-item { background: #1a1a2e; padding: 12px; border-radius: 10px; margin-bottom: 10px; border-left: 3px solid #10a37f; }
        .doc-item-id { color: #10a37f; font-size: 0.8rem; font-weight: bold; }
        ...
        .setting-item select { width: 100%; padding: 10px; background: #0f0f23; border: 1px solid #333; border-radius: 8px; color: white; font-size: 0.9rem; }
        
        /* 모델 정보 */
        .model-info { background: #1a1a2e; padding: 15px; border-radius: 10px; margin-bottom: 10px; border-left: 3px solid #10a37f; }
        .model-info h3 { color: #10a37f; font-size: 0.9rem; margin-bottom: 8px; }
        ...
        .badge-rag { background: rgba(16,163,127,0.2); color: #10a37f; border: 1px solid #10a37f; }
    ...
    <div class="app">
        ...
        <div class="chat-container" id="chatContainer">
            <div class="message bot">
                안녕하세요! 저는 OpenAI GPT 기반 AI 어시스턴트예요. 🤖<br><br>
                🎤 <b>음성 버튼</b>을 눌러 말하거나 텍스트로 입력하세요<br>
                📚 <b>RAG</b>: 저장된 문서에서 검색<br>
                🌐 <b>웹검색</b>: DuckDuckGo로 최신 정보 검색 (무료!)<br><br>
                무엇이든 물어보세요!
            </div>
        </div>
        ...
        <div class="tab-content" id="settingsTab">
            <div class="model-info">
                <h3>🧠 현재 모델</h3>
                <p id="modelName">GPT-4o</p>
                <p>OpenAI의 GPT + DuckDuckGo 웹검색을 사용합니다.</p>
            </div>
            ...
            <div class="setting-item">
                ...
                <select id="webSearchSetting">
                    <option value="true" selected>켜기 (최신 정보 검색 가능)</option>
    ...
    <script>
        ...
        async function checkHealth() {
            try {
                ...
                document.getElementById('statusText').textContent = data.llm_available ? 'OpenAI 연결됨' : 'API 키 필요';
                document.getElementById('docCount').textContent = data.documents || 0;
                document.getElementById('modelName').textContent = data.model || 'GPT-4o';
            } catch(e) {
...
def chat():
    ...
    # OpenAI API 호출 (웹 검색 포함/미포함)
    answer, web_sources = ask_openai_with_web_search(question, sources, use_web_search=use_web_search)
...
if __name__ == '__main__':
    print("\n" + "="*60)
    print("🎤 Voice RAG + OpenAI + 웹검색 (All-in-One)")
    print("="*60)
    ...
    print("   🌐 웹검색: DuckDuckGo 무료 검색 (API 키 불필요!)")
    
    if openai_client:
        print("\n✅ OpenAI API 키 설정됨")
    else:
        print("\n⚠️  OpenAI API 키가 없습니다!")
