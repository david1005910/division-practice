#!/usr/bin/env python3
"""
Voice RAG + OpenAI + Gmail + 웹검색 통합 시스템 (All-in-One)
...
주요 기능:
    ...
    5. 🌐 Web Search: DuckDuckGo 웹 검색 (최신 정보!)
...
def web_search_html(query, max_results=5):
    """HTML 스크래핑을 통한 웹 검색 (백업)"""
    try:
...
def needs_web_search(question):
    ...
    time_keywords = [
        ...
        '2024', '2025', '2026',
        '뉴스', '날씨', '주가', '환율', '주식', '가격',
        'today', 'now', 'current', 'recent', 'latest', 'news', 'weather'
    ]
...
def ask_openai_with_web_search(question, context_docs, use_web_search=False):
    ...
    if use_web_search:
        ...
        if not results:
            # 백업: HTML 스크래핑
            results, error = web_search_html(question)
        
        if results:
            web_results = results
            web_context = "\n\n=== 웹 검색 결과 ===\n"
            for i, r in enumerate(results, 1):
                web_context += f"[검색 {i}] {r['title']}\n{r['snippet']}\n\n"
...
<head>
    ...
    <title>🎤 Voice RAG + OpenAI + 📧메일 + 🌐웹검색</title>
    <style>
    ...
    <div class="app">
        ...
        <div class="chat-container" id="chatContainer">
            <div class="message bot">
                ...
                💡 <b>"메일 요약해줘"</b> 또는 <b>"오늘 뉴스"</b>라고 말해보세요!
            </div>
        </div>
        ...
        <div class="tab-content" id="settingsTab">
            <div class="model-info">
                ...
                <p>OpenAI GPT + 웹검색 + Gmail 메일 요약 지원</p>
            </div>
        ...
        <div class="input-container" id="inputContainer">
            <div class="quick-actions">
                <button class="quick-btn" onclick="quickMail()">📧 메일 요약</button>
...
def chat():
    ...
    # 웹 검색 자동 판단
    should_use_web = use_web_search and (needs_web_search(question) or not sources)
    
    answer, web_sources = ask_openai_with_web_search(question, sources, use_web_search=should_use_web)
